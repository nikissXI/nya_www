// 联机房间/游戏相关的公共常量与工具
// 注意：这些数据被多个组件（房间页、侧边栏等）共用，必须放在独立模块里。
// 如果直接放在 app/room/page.tsx 中导出，会让该页面被静态依赖，导致路由懒加载失效。
...
/** 把游戏配置里的文档路径转成后端使用的游戏名 */
export const getRoomGameName = (game?: GameRoomItem): string => {
